...
async def add_product_to_cart(product_id: int, user_email: str, database: Session = Depends(get_db)):
...
async def remove_product_from_cart(cart_item_id: int, database: Session = Depends(get_db)):
...
async def get_cart_by_user_email(email: str, database: Session = Depends(get_db)):
